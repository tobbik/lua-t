...
int lxt_pack_Sequence( lua_State *luaVM )
{
	...
	struct xt_pack     *p;
	struct xt_pack_seq *sq;

	// size = sizof(...) -1 because the array has already one member
	sz = sizeof( struct xt_pack_seq ) + (lua_gettop( luaVM ) - 1) * sizeof( struct xt_pack );
	sq = (struct xt_pack_seq *) lua_newuserdata( luaVM, sz );
	sq->n = lua_gettop( luaVM )-1;  // number of elements on stack -1 (the seq userdata)
	sq->buf_ref = LUA_NOREF;
	memset( sq->p, 0, sq->n * sizeof( struct xt_pack ));
	...
	sq->idx_ref = luaL_ref( luaVM, LUA_REGISTRYINDEX ); // keep registry ref for name:idx table
...
static int lxt_pack_seq__gc( lua_State *luaVM )
{
	...
	if (LUA_NOREF != sq->idx_ref)
		luaL_unref( luaVM, LUA_REGISTRYINDEX, sq->idx_ref ); // remove table at idx_ref from registry
	if (LUA_NOREF != sq->buf_ref)
		luaL_unref( luaVM, LUA_REGISTRYINDEX, sq->buf_ref ); // remove xt.Buffer at buf_ref from registry
	for (i=0; i < sq->n; i++)
	{
		sq->p[ i ].b = NULL;
	}
