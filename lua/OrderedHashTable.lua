...
local prxTblIdx, Table, equals =
      require( "t" ).proxyTableIndex, require( "t.Table" ), require"t.core".utl.equals
...
local t_keys,     t_values,     t_clone,     t_asstring =
      Table.keys, Table.values, Table.clone, Table.asstring

local _mt = { }

-- ---------------------------- general helpers  --------------------
-- assert Oht type and return the proxy table
local getPrx     = function( oht )
