...
t_buf_readbytes( size_t sz, int islittle, const unsigned char * buf )
{
	size_t         i;
 ...
 * --------------------------------------------------------------------------*/
static inline void
t_buf_writebytes( uint64_t val, size_t sz, int islittle, unsigned char * buf )
{
	size_t         i;
	unsigned char *set  = (unsigned char*) &val;  ///< char array to read bytewise into val
#ifndef IS_LITTLE_ENDIAN
 ...
 * --------------------------------------------------------------------------*/
static inline uint64_t
t_buf_readbits( size_t len, size_t ofs, const unsigned char * buf )
{
	uint64_t val = t_buf_readbytes( (len+ofs-1)/8 +1, 0, buf );
 ...
 * --------------------------------------------------------------------------*/
static inline void
t_buf_writebits( uint64_t val, size_t len, size_t ofs, unsigned char * buf )
{
	...
	read = t_buf_readbytes( abit/8, 0, buf );
	read = (val << (abit-ofs-len)) | (read & ~msk);
	t_buf_writebytes( read, abit/8, 0, buf);
...
t_pck_read( lua_State *luaVM, struct t_pck *p, const unsigned char *b)
{
	switch( p->t )
	{
		case T_PCK_INTL:
		case T_PCK_INTB:
			lua_pushinteger( luaVM, (lua_Integer) t_buf_readbytes( p->sz, p->t, b ) );
			break;
		case T_PCK_BYTE:
		...
		case T_PCK_BIT:
			...
			break;
		case T_PCK_BITS:
			lua_pushinteger( luaVM, (lua_Integer) t_buf_readbits( p->lB, p->oB - 1 , b ) );
			break;
		case T_PCK_STR:
...
t_pck_write( lua_State *luaVM, struct t_pck *p, unsigned char *b )
{
	...
	switch( p->t )
	{
		...
		case T_PCK_INTB:
			...
			t_buf_writebytes( (uint64_t) intVal, p->sz, p->t, b );
			break;
		case T_PCK_BYTE:
			intVal = luaL_checkinteger( luaVM, -1 );
		...
		case T_PCK_BITS:
			...
			luaL_argcheck( luaVM,  0 == (intVal >> (p->sz*8)) , -1,
			              "value to pack must be smaller than the maximum value for the packer size");
			t_buf_writebits( (uint64_t) intVal, p->lB, p->oB - 1, b );
			break;
		case T_PCK_STR:
...
lt_pck_Bits( lua_State *luaVM )
{
	...
	p = t_pck_create_ud( luaVM, T_PCK_BITS );
...
{
	lua_pushfstring( luaVM, "%s", t_pck_t_lst[ p->t ] );
	switch( p->t )
	{
		case T_PCK_INTL:
		case T_PCK_INTB:
...
luaopen_t_pck( lua_State *luaVM )
{
	...
	for (i=1; i<=8; i++)
	{
		...
		t     = t_pck_create_ud( luaVM, T_PCK_INTL );
		t->sz = i;
		lua_rawset( luaVM, -3 );
		lua_pushfstring( luaVM, "IntB%d", i );
		t     = t_pck_create_ud( luaVM, T_PCK_INTB );
		t->sz = i;
		t->iR = LUA_NOREF;
		lua_rawset( luaVM, -3 );
		// native endian
	...
	for (i=1; i<=8; i++)
	{
		...
		t = t_pck_create_ud( luaVM, T_PCK_BIT );
		t->sz  = 1;
		t->lB  = 1;
		t->oB  = i;
		t->iR = LUA_NOREF;
	...
	t = t_pck_create_ud( luaVM, T_PCK_NBL );
	t->oB  = 1;
	t->sz  = 1;
	t->iR = LUA_NOREF;
	lua_rawset( luaVM, -3 );
	...
	t = t_pck_create_ud( luaVM, T_PCK_BYTE );
	t->sz  = 1;
	t->iR = LUA_NOREF;
	lua_rawset( luaVM, -3 );
